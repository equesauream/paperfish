...
const std::map<char, int> Position::pieceMap = std::map<char, int>{
    std::pair<char, int>('P', 0),  std::pair<char, int>('N', 1),
    std::pair<char, int>('B', 2),  std::pair<char, int>('R', 3),
    std::pair<char, int>('Q', 4),  std::pair<char, int>('K', 5),
    std::pair<char, int>('p', 6),  std::pair<char, int>('n', 7),
    std::pair<char, int>('b', 8),  std::pair<char, int>('r', 9),
    std::pair<char, int>('q', 10), std::pair<char, int>('k', 11)};

const std::map<int, char> Position::intMap = std::map<int, char>{
    std::pair<int, char>(0, 'P'),  std::pair<int, char>(1, 'N'),
    std::pair<int, char>(2, 'B'),  std::pair<int, char>(3, 'R'),
    std::pair<int, char>(4, 'Q'),  std::pair<int, char>(5, 'K'),
    std::pair<int, char>(6, 'p'),  std::pair<int, char>(7, 'n'),
    std::pair<int, char>(8, 'b'),  std::pair<int, char>(9, 'r'),
    std::pair<int, char>(10, 'q'), std::pair<int, char>(11, 'k')};
...
Position::Position(const Position &other)
    : thePosition{other.thePosition}, whitePieces{other.whitePieces},
      blackPieces{other.blackPieces},

      OthePosition{other.OthePosition}, OwhitePieces{other.OwhitePieces},
      OblackPieces{other.OblackPieces},

      turn{other.turn},
      castleRights{other.castleRights}, enPassent{other.enPassent},
      halfMove{other.halfMove}, fullMove{other.fullMove},

      Oturn{other.Oturn}, OcastleRights{other.OcastleRights},
      OenPassent{other.OenPassent}, OhalfMove{other.OhalfMove},
...
NEXT:
  ...
  if (tolower(piece) == piece && (dest & blackPieces) != 0)
    return false;
  if (toupper(piece) == piece && (dest & whitePieces) != 0)
    return false;

  if (piece == 'p') {
    if (colNumber(source) != colNumber(dest) &&
        rowNumber(source) != rowNumber(dest)) {
      if (dest != enPassent && (dest & whitePieces) == 0)
        return false;
    } else {
      if ((dest & blackPieces) != 0 || (dest & whitePieces) != 0)
        return false;
    }
  } else if (piece == 'P') {
    if (colNumber(source) != colNumber(dest) &&
        rowNumber(source) != rowNumber(dest)) {
      if (dest != enPassent && (dest & blackPieces) == 0)
        return false;
    } else {
      if ((dest & whitePieces) != 0 || (dest & blackPieces) != 0)
        return false;
    }
  }
  ...
  if (piece == 'K') {
    ...
    if (enemyKing.at(0) - colNumber(dest) <= 1 &&
        enemyKing.at(0) - colNumber(dest) >= -1 &&
        enemyKing.at(1) - rowNumber(dest) <= 1 &&
        enemyKing.at(1) - rowNumber(dest) >= -1) {
      return false;
    }
    ...
    if (dest == C1 && source == E1) {
      if ((B1 & blackPieces) != 0 || (B1 & whitePieces) != 0)
        return false;
    }
    ...
    if ((dest == G1 && source == E1) || (dest == C1 && source == E1)) {
      ...
      if (!((thePosition.at(pieceMap.at('K'))) & king) ||
          !((thePosition.at(pieceMap.at('R'))) & rook))
        return false;

      if ((kpos & blackPieces) != 0 || (rpos & blackPieces) != 0)
        return false;
      if ((kpos & whitePieces) != 0 || (rpos & whitePieces) != 0)
        return false;
    ...
    if (dest == C8 && source == E8) {
      if ((B8 & blackPieces) != 0 || (B8 & whitePieces) != 0)
        return false;
    }
  ...
  if (isMoveBlocked(Move(source, dest, orig, prom)))
    return false;
...
bool Position::isMoveBlocked(const Move &m) const {
  ...
  if (colNumber(source) == colNumber(dest) &&
    ...
    cur >>= 8;
    while (cur != dest) {
      if ((cur & blackPieces) != 0 || (cur & whitePieces) != 0)
        return true;
      cur >>= 8;
    }
  } else if (colNumber(source) == colNumber(dest) &&
             rowNumber(source) >= rowNumber(dest)) {
    cur <<= 8;
    while (cur != dest) {
      if ((cur & blackPieces) != 0 || (cur & whitePieces) != 0)
...
std::vector<Move> Position::possibleMoves(char p, Square pos) const {
  if (p == 'p') {
    ...
    if (rowNumber(pos) == 7 && colNumber(pos) < 'h' && colNumber(pos) > 'a') {
      tmp =
          std::vector<Move>{Move(pos, pos << 8, 'p'), Move(pos, pos << 16, 'p'),
                            Move(pos, pos << 7, 'p'), Move(pos, pos << 9, 'p')};
      ...
      tmp =
          std::vector<Move>{Move(pos, pos << 8, 'p'), Move(pos, pos << 16, 'p'),
               ...
               colNumber(pos) > 'a') {
      tmp = std::vector<Move>{
          Move(pos, pos << 8, 'p', 'n'), Move(pos, pos << 8, 'p', 'b'),
          Move(pos, pos << 8, 'p', 'r'), Move(pos, pos << 8, 'p', 'q'),

          Move(pos, pos << 7, 'p', 'n'), Move(pos, pos << 7, 'p', 'b'),
          Move(pos, pos << 7, 'p', 'r'), Move(pos, pos << 7, 'p', 'q'),

          Move(pos, pos << 9, 'p', 'n'), Move(pos, pos << 9, 'p', 'b'),
          Move(pos, pos << 9, 'p', 'r'), Move(pos, pos << 9, 'p', 'q'),
      };
      ...
      tmp =
          std::vector<Move>{Move(pos, pos << 8, 'p'), Move(pos, pos << 7, 'p'),
                            Move(pos, pos << 9, 'p')};
    } else if (rowNumber(pos) <= 6 && rowNumber(pos) >= 3 &&
               colNumber(pos) == 'h') {
      tmp =
          std::vector<Move>{Move(pos, pos << 8, 'p'), Move(pos, pos << 7, 'p')};
               ...
               colNumber(pos) == 'a') {
      tmp =
          std::vector<Move>{Move(pos, pos << 8, 'p'), Move(pos, pos << 9, 'p')};
    } else {
    ...
    if (toupper(p) == p) {
      if (castleRights & (1 << 3) && pos == E1)
        tmp.push_back(Move(E1, G1, 'K'));
      if (castleRights & (1 << 2) && pos == E1)
        tmp.push_back(Move(E1, C1, 'K'));
    } else if (tolower(p) == p) {
      if (castleRights & (1 << 1) && pos == E8)
        tmp.push_back(Move(E8, G8, 'k'));
      if (castleRights & (1 << 0) && pos == E8)
        tmp.push_back(Move(E8, C8, 'k'));
    }
    return tmp;
  }
...
std::vector<Move> Position::legalMoves() {
  std::vector<Move> tmp;
  for (int i = 0; i < 12; ++i) {
    if ((i <= 5 && turn == true) || (i >= 6 && turn == false))
      continue;
...
int Position::materialCount() const {
  ...
  std::map<char, int> values = std::map<char, int>{
      std::pair<char, int>('P', 1), std::pair<char, int>('N', 3),
      std::pair<char, int>('B', 3), std::pair<char, int>('R', 5),
      std::pair<char, int>('Q', 9), std::pair<char, int>('p', 1),
      std::pair<char, int>('n', 3), std::pair<char, int>('b', 3),
      std::pair<char, int>('r', 5), std::pair<char, int>('q', 9)};
...
void Position::updateBoards() {
  whitePieces = thePosition.at(0) | thePosition.at(1) | thePosition.at(2) |
                thePosition.at(3) | thePosition.at(4) | thePosition.at(5);

  blackPieces = thePosition.at(6) | thePosition.at(7) | thePosition.at(8) |
                thePosition.at(9) | thePosition.at(10) | thePosition.at(11);
}
