...
int rowNumber(Square s) { return bitToSquare(s).at(1) - '0'; }

char colNumber(Square s) { return bitToSquare(s).at(0); }
...
std::string bitToSquare(U64 bit) {
  ...
  --pos;
  return std::string(1, char('h' - ((pos % 8)))) +
         std::to_string((pos / 8) + 1);
}
